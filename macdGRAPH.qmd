...
peak_tbl <- data.frame(
  種別   = "山（頂点）",
  日付   = df_recent$date[peak_idx],
  スプライン値 = round(spline_y[peak_idx], 5)
)

valley_tbl <- data.frame(
  種別   = "谷（底）",
  日付   = df_recent$date[valley_idx],
  スプライン値 = round(spline_y[valley_idx], 5)
)

result_tbl <- rbind(peak_tbl, valley_tbl)
result_tbl <- result_tbl[order(result_tbl$日付), ]
